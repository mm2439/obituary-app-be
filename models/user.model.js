...
const jwt = require("jsonwebtoken"); // (kept for backward-compat generateAccessToken/RefreshToken if referenced elsewhere)
const Joi = require("joi");

const { sequelize } = require("../startup/db");

/** ---------- helpers for slug generation (unchanged) ---------- */
...
User.init(
  {
    ...
    // kept for backward compatibility; not used for auth anymore
    password: {
      type: DataTypes.TEXT,
      allowNull: true,
    },

    // NEW: link to Supabase Auth user id (uuid from Supabase)
    supabase_user_id: {
    ...
    createObituaryPermission: { type: DataTypes.BOOLEAN, defaultValue: false },
    assignKeeperPermission: { type: DataTypes.BOOLEAN, defaultValue: false },
    sendGiftsPermission: { type: DataTypes.BOOLEAN, defaultValue: false },
    sendMobilePermission: { type: DataTypes.BOOLEAN, defaultValue: false },

    isBlocked: { type: DataTypes.BOOLEAN, allowNull: false, defaultValue: false },
    notes: { type: DataTypes.TEXT, allowNull: true, validate: { len: [0, 500] } },
    adminRating: { type: DataTypes.STRING(1), allowNull: true, validate: { len: [0, 1] } },
    hasFlorist: { type: DataTypes.BOOLEAN, allowNull: false, defaultValue: false },
...
/** -------- remove password hashing; Supabase owns credentials now -------- */
// (No User.beforeCreate bcrypt hook)

/** -------- keep your slugKey generator logic (unchanged) -------- */
 ...
 * With Supabase-only auth, prefer verifying Supabase JWTs in middleware.
 */
...
function validateRegister(user) {
  const schema = Joi.object({
    ...
    password: Joi.string().required(), // required for Supabase user creation
...
function validateUser(user) {
  const userSchema = Joi.object({
    ...
    // forbid local password updates; Supabase owns credentials
    password: Joi.any().forbidden(),
    company: Joi.string().max(100).optional().allow(null, ""),
    region: Joi.string().max(100).optional().allow(null, ""),
    city: Joi.string().max(100).optional().allow(null, ""),
    secondaryCity: Joi.string().max(100).optional().allow(null, ""),
    role: Joi.string()
      .valid(
        process.env.USER_ROLE,
        process.env.FUNERAL_COMPANY_ROLE,
...
module.exports = {
  User,
  generateAccessToken, // deprecated
  generateRefreshToken, // deprecated
  validateRegister,
  validateUser,
};
